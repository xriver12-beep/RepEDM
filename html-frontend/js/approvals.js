...
class ApprovalsManager {
    ...
    async setupComponents() {
        ...
        // Modals initialized manually when needed
    }
    ...
    setupEventListeners() {
        ...
        document.addEventListener('click', (e) => {
            if (e.target.classList.contains('modal')) {
                // 如果設定了 data-backdrop="static"，則點擊背景不關閉
                // 優先使用 getAttribute 確保讀取正確
                const backdrop = e.target.getAttribute('data-backdrop') || e.target.dataset.backdrop;
                if (backdrop === 'static') {
                    return;
                }
                this.closeModal();
        ...
        document.addEventListener('click', (e) => {
            ...
            if (target.closest('#approveBtn')) {
                this.openDecisionModal('approve');
            } else if (target.closest('#revisionBtn')) {
                this.openDecisionModal('revision');
            } else if (target.closest('#rejectBtn')) {
                this.openDecisionModal('reject');
            } else if (target.closest('#confirmDecisionBtn')) {
                this.confirmDecision();
            } else if (target.closest('#cancelDecisionBtn')) {
                this.closeModal();
        ...
        document.addEventListener('click', (e) => {
            if (e.target.id === 'previewModeBtn') {
                ...
                if (approval) {
                    this.openPreviewWindow(approval.content);
                }
            }
    ...
    generateMockApprovalData() {
        ...
        return Array.from({ length: 5 }, (_, i) => {
            ...
            return {
                ...
                scheduled_time: new Date(createdDate.getTime() + 2 * 24 * 60 * 60 * 1000),
                ...
                recipient_count: Math.floor(Math.random() * 5000) + 100 // Generate random recipient count
            };
        });
    }
    ...
    async viewApprovalDetail(id) {
        ...
        if (modal) {
            ...
            modal.style.display = 'flex'; // Ensure display is visible (overriding any inline none)
            // 保存原始 ID (可能是 UUID 格式)
            ...
            try {
                ...
                if (response.success && response.data && response.data.approval) {
                    ...
                    const fullApproval = {
                        ...
                        scheduled_time: detail.scheduled_at || detail.ScheduledAt || detail.scheduledAt || detail.ScheduledTime || detail.send_at || detail.SendAt,
    ...
    clearDetailModal() {
        ...
        const scheduledTimeEl = document.getElementById('detailScheduledTime');
    ...
    renderApprovalDetailModal(approval) {
        ...
        safeText('detailScheduledTime', approval.scheduled_time ? FormatUtils.formatDate(approval.scheduled_time, 'YYYY-MM-DD HH:mm') : '立即發送');
    ...
    updateContentPreview(content) {
        ...
        const isSourceMode = sourceCodeBtn && sourceCodeBtn.classList.contains('active');

        if (!content) {
            previewContainer.innerHTML = '<div class="no-content">暫無內容預覽</div>';
            return;
        }

        if (isSourceMode) {
            const safeContent = content.replace(/</g, '&lt;').replace(/>/g, '&gt;');
            previewContainer.innerHTML = `<pre class="approval-source-view"><code>${safeContent}</code></pre>`;
            return;
        }

        previewContainer.innerHTML = '<iframe id="contentPreviewFrame" class="approval-preview-frame"></iframe>';
        const iframe = document.getElementById('contentPreviewFrame');
        if (!iframe) return;

        let htmlContent = content.trim();
        const hasFullDocument = /<!doctype/i.test(htmlContent) || /<html[\s>]/i.test(htmlContent);
    ...
    switchTab(tabName) {
        ...
        // Auto-Fullscreen logic for Content Preview tab
        const modal = document.getElementById('approvalDetailModal');
        if (modal) {
            ...
            if (tabName === 'content') {
                ...
                // Ensure content is rendered (sometimes switching tabs might need re-render if iframe is cleared)
                // Although switchPreviewMode handles rendering, sometimes we just switch tab
                const previewContainer = document.getElementById('contentPreview');
                ...
                // Exit fullscreen if active when switching to other tabs
                if (isFullscreen) {
                    this.toggleFullScreen();
                }
            }
    ...
    async confirmDecision() {
        ...
        try {
            ...
            // Check force send
            const forceSendCheck = document.getElementById('forceSendCheck');
            const forceSend = forceSendCheck ? forceSendCheck.checked : false;
...
<body>
    ${htmlContent}
            ...
            NotificationUtils.show('無法開啟新視窗，請檢查瀏覽器設定', 'warning');
        }
    }
