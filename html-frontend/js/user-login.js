...
class UserLoginManager {
    ...
    loadRememberedUsername() {
        try {
            const rememberedUsername = localStorage.getItem('rememberedUserUsername');
            if (rememberedUsername) {
                this.usernameInput.value = rememberedUsername;
                this.rememberMeCheckbox.checked = true;
            }
        } catch (error) {
            console.warn('無法載入記住的用戶名:', error);
        }
    }
    ...
    async handleSubmit(e) {
        ...
        try {
            ...
            try {
                if (rememberMe) {
                    localStorage.setItem('rememberedUserUsername', username);
                } else {
                    localStorage.removeItem('rememberedUserUsername');
                }
            } catch (error) {
                console.warn('無法保存記住我設定:', error);
            }

            // 登入成功，重定向到前台儀表板
            this.showSuccessMessage('登入成功！正在進入系統...');
