...
class AnalyticsManager {
    ...
    async loadData(forceRefresh = false) {
        ...
        try {
            ...
            // 檢查緩存
            const cacheKey = `analytics_v2_${this.dateRange}`;
    ...
    async fetchAnalyticsData() {
        try {
            ...
            const [dashboardData, campaignsData, trendData, devicesData, timeData, geoData] = await Promise.all([
                fetchOptional(window.apiClient.get('/analytics/dashboard', { _t: Date.now() }), { totalSent: 0, openRate: 0, clickRate: 0 }),
                fetchOptional(window.apiClient.get('/campaigns', { limit: 5, _t: Date.now() }), []),
                ...
                // 改為獲取發送郵件的網域統計
                fetchOptional(window.apiClient.get('/analytics/sent-domains', { _t: Date.now() }), { data: [] })
            ]);
    ...
    async viewCampaignDetail(id) {
        ...
        if (tbodyDomains) {
            tbodyDomains.innerHTML = '<tr><td colspan="6" class="text-center"><i class="fas fa-spinner fa-spin me-2"></i>載入詳細數據中...</td></tr>';
        }
        ...
        try {
            ...
            if (tbodyDomains) {
                tbodyDomains.innerHTML = '<tr><td colspan="6" class="text-center text-danger">無法載入詳細數據</td></tr>';
            }
            if (tbodyFailures) {
    ...
    updateDomainsTable(domains) {
        ...
        if (!domains || domains.length === 0) {
             tbody.innerHTML = '<tr><td colspan="6" class="text-center text-muted">無網域統計數據</td></tr>';
             return;
        }
    ...
    updateFailuresTable(failures) {
        ...
        tbody.innerHTML = failures.map(item => {
            ...
            <tr>
                <td>${item.domain || 'Unknown'}</td>
                <td class="text-danger" style="word-break: break-all;">${item.reason || 'Unknown Error'}</td>
    ...
    async exportReport() {
        try {
            ...
            a.href = url;
            a.download = `analytics_dashboard_${this.dateRange}_${new Date().toISOString().split('T')[0]}.html`;
            document.body.appendChild(a);
            a.click();
            document.body.removeChild(a);
            window.URL.revokeObjectURL(url);
            
            NotificationUtils.show('綜合分析報告匯出成功', 'success');
...
<body>
    ...
    async exportCampaignReport(id) {
        try {
            ...
            const a = document.createElement('a');
            a.href = url;
            a.download = `campaign_report_${campaign.id}_${new Date().toISOString().split('T')[0]}.html`;
            document.body.appendChild(a);
            a.click();
            document.body.removeChild(a);
            window.URL.revokeObjectURL(url);
            
            NotificationUtils.show('活動分析報告匯出成功', 'success');
