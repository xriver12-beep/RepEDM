...
router.get('/',
  ...
  asyncHandler(async (req, res) => {
    ...
    // Geo Filter Logic
    const buildGeoFilter = (countryVal, cityVal) => {
        let clauses = [];
        const p = {};
        const parse = (str) => str ? str.split(',').map(s => s.trim()).filter(s => s) : [];

        const countries = parse(countryVal);
        if (countries.length > 0) {
            const pNames = countries.map((_, i) => `g_country_${i}`);
            clauses.push(`s.country IN (${pNames.map(name => '@' + name).join(',')})`);
            countries.forEach((c, i) => p[`g_country_${i}`] = c);
        }

        const cities = parse(cityVal);
        if (cities.length > 0) {
             const pNames = cities.map((_, i) => `g_city_${i}`);
             clauses.push(`s.city IN (${pNames.map(name => '@' + name).join(',')})`);
             cities.forEach((c, i) => p[`g_city_${i}`] = c);
        }
    ...
    // 獲取活動列表
    let recipientCountCol = 'c.recipient_count';
    let openedCountCol = 'c.opened_count';
    let clickedCountCol = 'c.clicked_count';
    // bounced/unsubscribed logic could also be filtered, but keeping simple for now unless requested
    
    let cappedCountCol = "(SELECT COUNT(*) FROM EmailSends WHERE campaign_id = c.id AND status = 'capped')";

    if (geoFilter.hasFilter) {
        recipientCountCol = `(SELECT COUNT(*) FROM EmailSends es JOIN Subscribers s ON es.subscriber_id = s.id WHERE es.campaign_id = c.id ${geoFilter.where})`;
        openedCountCol = `(SELECT COUNT(*) FROM EmailOpens eo JOIN Subscribers s ON eo.SubscriberID = s.id WHERE eo.CampaignID = c.id ${geoFilter.where})`;
        clickedCountCol = `(SELECT COUNT(*) FROM EmailClicks ec JOIN Subscribers s ON ec.SubscriberID = s.id WHERE ec.CampaignID = c.id ${geoFilter.where})`;
        cappedCountCol = `(SELECT COUNT(*) FROM EmailSends es JOIN Subscribers s ON es.subscriber_id = s.id WHERE es.campaign_id = c.id AND es.status = 'capped' ${geoFilter.where})`;
    }
    ...
    const query = `
      SELECT 
        ...
        ${recipientCountCol} as recipient_count, 
        ${openedCountCol} as opened_count, 
        ${clickedCountCol} as clicked_count, 
        ...
        ${cappedCountCol} as capped_count
      FROM Campaigns c
    ...
    const campaigns = await Promise.all(result.recordset.map(async campaign => {
        ...
        // Calculate details if it's a category audience, BUT ONLY if not sent yet
        // For sent campaigns, we rely on historical recipient_count
        const isSent = ['sent', 'completed'].includes(campaign.status);
        if (!isSent && (campaign.target_audience || '').toLowerCase() === 'category') {
             try {
        ...
        return {
          ...
          stats: {
            ...
            cappedCount: campaign.capped_count || 0,
...
router.post('/preview-stats', 
  asyncHandler(async (req, res) => {
    const { targetAudience, targetFilter, forceSend } = req.body;
    
    // 模擬一個 Campaign 對象供 recipientService 使用
    const mockCampaign = {
        target_audience: targetAudience,
        target_filter: typeof targetFilter === 'string' ? targetFilter : JSON.stringify(targetFilter)
    };
    
    // 1. 計算潛在受眾
    const recipients = await recipientService.getRecipients(mockCampaign);
    const total = recipients.length;
    
    // 2. 計算頻率限制
    const subscriberIds = recipients.filter(r => r.id).map(r => r.id);
    const { capped } = await frequencyService.checkFrequency(subscriberIds, forceSend === true);
    const cappedCount = capped.length;
...
router.get('/:id',
  asyncHandler(async (req, res) => {
    ...
    // Calculate Frequency Capping Stats (Simulation for non-sent campaigns)
    let cappedCount = 0;
    let expectedRecipients = campaign.recipient_count || 0;

    if (!['sent', 'completed', 'processing', 'sending'].includes(campaign.status)) {
        try {
            // Fetch potential recipients
            const recipients = await recipientService.getRecipients(campaign);
            expectedRecipients = recipients.length; // Update with accurate current count
            
            // Check frequency capping
            const subscriberIds = recipients.filter(r => r.id).map(r => r.id);
            const forceSend = campaign.force_send === true || campaign.force_send === 1;
            const { capped } = await frequencyService.checkFrequency(subscriberIds, forceSend);
            cappedCount = capped.length;
        } catch (e) {
            console.error('Error calculating frequency stats:', e);
        }
    } else {
        // For sent campaigns, query actual capped count from EmailSends (if we record it there)
        // Or if we just rely on the stored count if we updated it?
        // We updated recipient_count in scheduler, but didn't store capped_count explicitly in Campaigns table.
        // We stored it in EmailSends with status 'capped'.
        const cappedQuery = `SELECT COUNT(*) as count FROM EmailSends WHERE campaign_id = @id AND status = 'capped'`;
        const cappedRes = await executeQuery(cappedQuery, { id });
        cappedCount = cappedRes.recordset[0].count;
    }
    ...
    res.json({
      ...
      data: {
        ...
        name: campaign.name,
        subject: campaign.subject,
        force_send: campaign.force_send, // Return force_send flag
        capped_count: cappedCount,       // Return capped count
        recipient_count: expectedRecipients, // Return accurate recipient count
        effective_recipient_count: expectedRecipients - cappedCount, // Count after capping
  ...
  asyncHandler(async (req, res) => {
    ...
    const insertQuery = `
      INSERT INTO Campaigns (
        ...
        track_opens, track_clicks, include_unsubscribe, preview_text, force_send
      )
      OUTPUT INSERTED.id
      VALUES (
        ...
        @trackOpens, @trackClicks, @includeUnsubscribe, @previewText, @forceSend
      )
    `;
    ...
    const result = await executeQuery(insertQuery, {
      ...
      forceSend: req.body.forceSend ? 1 : 0
    });
