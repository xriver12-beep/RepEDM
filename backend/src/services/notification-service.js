...
class NotificationService {
     ...
     * @param {number} approvalId 
     */
    async sendReminderNotification(approvalId) {
        try {
            // 1. 獲取審核詳情和審批人資訊 (重用查詢)
            const query = `
                SELECT 
            ...
            if (result.recordset.length === 0) {
                console.error(`NotificationService: 找不到審核記錄 ${approvalId} (Reminder)`);
                return;
            }
            ...
            // 2. 確定收件人 (與 sendApprovalNotification 邏輯相同)
            if (item.approver_type === 'Manager') {
                if (item.SubmitterManagerID) {
                    const managerQuery = `SELECT id, email, full_name FROM Users WHERE id = @id`;
                    const managerResult = await executeQuery(managerQuery, { id: item.SubmitterManagerID });
            ...
            if (recipients.length === 0) {
                console.warn(`NotificationService: 審核記錄 ${approvalId} 沒有找到有效的審批人 (Reminder)`);
                return;
            }
            ...
            for (const recipient of recipients) {
                ...
                if (token) {
                    ...
                    magicLinkHtml = `
                        <div style="text-align: center; margin: 30px 0;">
                            <a href="${link}" 
                               style="background-color: #ffc107; color: #333; padding: 12px 24px; text-decoration: none; border-radius: 5px; font-weight: bold; font-size: 16px;">
                                🔔 立即審核 (免登入)
                            </a>
                            <p style="margin-top: 10px; font-size: 12px; color: #888;">連結有效期為 24 小時</p>
                        </div>
                ...
                const subject = `[逾期提醒] 活動 "${item.CampaignName}" 等待您的審核`;
                
                const html = `
                    <div style="font-family: Arial, sans-serif; width: 100%; max-width: 600px; margin: 0 auto;">
                        <h2 style="color: #d39e00;">⏰ 審核逾期提醒</h2>
                        <p>親愛的 ${recipient.full_name}，</p>
                        <p>這是一個友善的提醒，<strong>${item.SubmitterName}</strong> 提交的活動需要您的審核。</p>
                        
                        <div style="background-color: #fff3cd; padding: 15px; border-radius: 5px; margin: 20px 0; border: 1px solid #ffeeba;">
                            <p style="margin: 5px 0;"><strong>活動名稱：</strong> ${item.CampaignName}</p>
                            <p style="margin: 5px 0;"><strong>郵件主旨：</strong> ${item.CampaignSubject}</p>
                            <p style="margin: 5px 0;"><strong>當前步驟：</strong> ${item.StepName}</p>
                            <p style="margin: 5px 0; color: #856404; font-weight: bold;">此審核項目已經等待一段時間，請盡快處理。</p>
                        </div>
                ...
                await emailService.sendEmail({
                    ...
                    subject: subject,
                    html: html,
                    text: `[逾期提醒] 活動 "${item.CampaignName}" 等待您的審核。請盡快處理。`,
                    emailType: 'approval_reminder'
                });
                
                console.log(`NotificationService: 已發送審核提醒通知給 ${recipient.email}`);
            }

            // 更新最後提醒時間
            await executeQuery(`
                UPDATE ApprovalItems 
                SET last_reminded_at = GETDATE() 
                WHERE id = @approvalId
            `, { approvalId });
            
        } catch (error) {
            console.error('NotificationService: 發送審核提醒通知失敗', error);
        }
    }
     ...
     * 發送活動成效報告給建立者
     */
    async sendCampaignReport(campaignId, recipientEmail, isUpdate = false) {
        try {
            // 1. 獲取活動詳情
            const campaignQuery = `
                SELECT 
            ...
            // 根據是否為更新報告調整主旨與標題
            const subjectPrefix = isUpdate ? '[即時進度更新]' : '[發送完成通知]';
            const actionText = isUpdate ? '最新成效報告' : '已發送完成';
            const subject = `${subjectPrefix} 活動 "${campaign.name}" ${actionText}`;
            
            const sentDate = campaign.sent_at ? new Date(campaign.sent_at).toLocaleString('zh-TW') : '尚未發送';
            
            const title = isUpdate ? '活動即時進度報告' : '活動發送完成通知';
            const intro = isUpdate 
                ? `這是您要求的活動 <strong>${campaign.name}</strong> 最新即時進度報告（數據更新時間：${new Date().toLocaleString('zh-TW')}）。` 
                : `您的活動 <strong>${campaign.name}</strong> 已於 <strong>${sentDate}</strong> 成功發送完成。`;

            // 僅在非更新報告（即首次發送完成通知）時顯示 0% 說明，避免更新報告時誤導
            const noticeBlock = isUpdate ? '' : `
                <p style="background-color: #e8f4fd; padding: 10px; border-radius: 4px; font-size: 14px; color: #0c5460; border-left: 4px solid #17a2b8;">
                    ℹ️ <strong>說明：</strong> 以下為發送完成當下的即時初始數據。由於郵件剛寄出，收件者可能尚未開啟或點擊，因此數據目前顯示為 0.0% 是<strong>正常現象</strong>，請無需擔心。
                </p>`;

            const html = `
                <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; color: #333;">
                    <h2 style="color: #0056b3; border-bottom: 2px solid #0056b3; padding-bottom: 10px;">${title}</h2>
                    <p>親愛的 ${campaign.creator_name || '用戶'}，</p>
                    <p>${intro}</p>
                    ${noticeBlock}

                    <div style="text-align: center; margin: 20px 0;">
                        <a href="${process.env.BACKEND_URL || 'https://edm2022.winton.com.tw:3443'}/api/tracking/report/${campaign.id}" 
                           style="background-color: #0056b3; color: white; padding: 12px 25px; text-decoration: none; border-radius: 5px; font-weight: bold; display: inline-block; margin-right: 10px;">
                           查看最新即時進度 (發送更新報告)
                        </a>
                        <a href="${process.env.BACKEND_URL || 'https://edm2022.winton.com.tw:3443'}/api/tracking/analysis/${campaign.id}" 
                    ...
                    <div style="margin: 20px 0;">
                        <h3 style="color: #495057;">核心指標 (初始數據)</h3>
                        ...
                        <p style="font-size: 12px; color: #888; text-align: center; margin-top: 5px;">* 數據將隨使用者互動即時更新</p>
                    </div>
                    ...
                    <div style="margin: 25px 0; padding: 15px; background-color: #fcfcfc; border: 1px dashed #ccc; border-radius: 5px;">
                        <h4 style="margin-top: 0; color: #555;">📈 數據追蹤小撇步：</h4>
                        <ul style="color: #666; font-size: 14px; padding-left: 20px;">
                            <li style="margin-bottom: 5px;"><strong>即時更新</strong>：當收件者開啟郵件或點擊連結時，系統會自動更新數據。</li>
                            <li style="margin-bottom: 5px;"><strong>觀察時機</strong>：建議於發送後 <strong>1-2 小時</strong> 或 <strong>24 小時</strong> 後查看，通常會有較顯著的數據變化。</li>
                        </ul>
                    </div>
                </div>
            `;
