...
class EmailService {
    ...
    // Helper to generate a simple hash of settings to detect changes
    ...
    async createTransporter(smtpSettings = null) {
        try {
            ...
            const newSettingsHash = this.getSettingsHash(settings);
            if (this.transporter && this.currentSettingsHash === newSettingsHash) {
                // Settings haven't changed, verify if transporter is still valid (optional, but good)
                if (this.transporter.isIdle && !this.transporter.isIdle()) {
                     // If needed, we could check status, but nodemailer pool manages this well.
                     // Just return existing transporter.
                }
                return this.transporter;
            }

            // 準備 DKIM 配置
            // 改由 PMG (Proxmox Mail Gateway) 負責簽章，此處移除應用層簽章以避免衝突
            let dkimConfig = null;
            
            this.transporter = nodemailer.createTransport({
                pool: true, // Enable connection pooling
                maxConnections: 5, // Limit concurrent connections to avoid overwhelming SMTP server
            ...
            this.currentSettingsHash = newSettingsHash;
            return this.transporter;
        } catch (error) {
    ...
    // 發送一般郵件
    async sendEmail({ to, subject, html, text, emailType = 'notification', attachments = [], headers = {}, campaignId = null }) {
        const startTime = Date.now();
        let settings = null;
        let emailSettings = {};
        ...
        try {
            ...
            const mailOptions = {
                ...
                headers: headers // Add custom headers support
            };
